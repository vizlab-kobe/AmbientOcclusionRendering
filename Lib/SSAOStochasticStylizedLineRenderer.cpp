...
void SSAOStochasticStylizedLineRenderer::Engine::setup( kvs::ObjectBase* object, kvs::Camera* camera, kvs::Light* light )
{
    ...
    kvs::ProgramObject::Binder bind( m_ao_buffer.geometryPassShader() );
...
//    m_ao_buffer.geometryPassShader().setUniform( "random_texture", 2 );
    ...
    m_ao_buffer.geometryPassShader().setUniform( "edge_factor", m_edge_factor );
...
void SSAOStochasticStylizedLineRenderer::Engine::update_buffer_object( const kvs::LineObject* line )
{
    m_buffer_object.release();
    this->create_buffer_object( line );
}
