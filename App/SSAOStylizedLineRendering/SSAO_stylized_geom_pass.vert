...
void main()
{
    vec4 p = gl_ModelViewMatrix * gl_Vertex; // vertex in world coordinate
    vec3 v = normalize( -p.xyz ); // vector from camera to the vertex
    vec3 t = gl_NormalMatrix * gl_Normal; // tangent in world coordinate
    ...
    vec4 mp = gl_ProjectionMatrix * vec4( p.xyz + v * depth, 1.0 );
