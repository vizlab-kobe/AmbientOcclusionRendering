...
struct Input
{
    enum Method { Raycasting = 0, Streamline = 1 };
    ...
    {
        kvs::ColorMap::CoolWarm( 256 ), // for raycasting
        kvs::ColorMap::BrewerSpectral( 256 )  // for streamline
    }; ///< transfer functions
    std::string labels[2] = { " ", " " }; ///< value label
    std::string filenames[2] = { "", "" }; ///< input filenames for raycasting and streamline
    ...
    bool parse( int argc, char** argv )
    {
        ...
        cl.addOption( "raycasting_tfunc", "Transfer function file for raycasting (default: brewer spectral).", 1, false );
        cl.addOption( "streamline_tfunc", "Transfer function file for streamline (default: brewer spectral).", 1, false );
        cl.addOption( "raycasting_label", "Value label for raycasting (default: "").", 1, false );
        cl.addOption( "streamline_label", "Value label for streamline (default: "").", 1, false );
        cl.addOption( "raycasting_file", "Input file for raycasting (*.kvsml).", 1, true );
        ...
        if ( cl.hasOption("raycasting_tfunc") ) tfuncs[Raycasting] = kvs::TransferFunction( cl.optionValue<std::string>("raycasting_tfunc") );
        if ( cl.hasOption("streamline_tfunc") ) tfuncs[Streamline] = kvs::TransferFunction( cl.optionValue<std::string>("streamline_tfunc") );
        if ( cl.hasOption("raycasting_label") ) labels[Raycasting] = cl.optionValue<std::string>("raycasting_label");
        if ( cl.hasOption("streamline_label") ) labels[Streamline] = cl.optionValue<std::string>("streamline_label");
        if ( cl.hasOption("raycasting_file") ) filenames[Raycasting] = cl.optionValue<std::string>("raycasting_file");
