...
private:
    ...
    kvs::ColorMapBar m_cmap_bars[2] =
    {
        kvs::ColorMapBar( &m_view.screen() ),
        kvs::ColorMapBar( &m_view.screen() )
    };
    kvs::TransferFunctionEditor m_editors[2] =
    {
        kvs::TransferFunctionEditor( &m_view.screen() ),
        kvs::TransferFunctionEditor( &m_view.screen() )
    };
...
public:
        ...
        m_view( view )
    {
        ...
        auto* raycasting_volume = m_model.import( Input::Method::Raycasting );
        const auto raycasting_min_value = raycasting_volume->minValue();
        const auto raycasting_max_value = raycasting_volume->maxValue();

        // Initial values for streamline.
        auto* streamline_volume = m_model.import( Input::Method::Streamline );
        ...
        m_edge_slider.sliderReleased( [&] ()
        {
            ...
            // Raycasting renderer
            {
                using Renderer = local::Model::AORaycastingRenderer;
                auto* renderer = Renderer::DownCast( scene->renderer( "RaycastingRenderer" ) );
        ...
        // Colormap bar
        const auto raycasting_cmap = m_model.tfuncs[Model::Method::Raycasting].colorMap();
        auto& raycasting_cmap_bar = m_cmap_bars[Model::Method::Raycasting];
        raycasting_cmap_bar.setCaption( m_model.labels[Model::Method::Raycasting] );
        raycasting_cmap_bar.setColorMap( raycasting_cmap );
        raycasting_cmap_bar.setRange( raycasting_min_value, raycasting_max_value );

        const auto streamline_cmap = m_model.tfuncs[Model::Method::Streamline].colorMap();
        auto& streamline_cmap_bar = m_cmap_bars[Model::Method::Streamline];
        streamline_cmap_bar.setCaption( m_model.labels[Model::Method::Streamline] );
        ...
        auto& raycasting_editor = m_editors[Model::Method::Raycasting];
        raycasting_editor.setTransferFunction( m_model.tfuncs[Model::Method::Raycasting] );
        raycasting_editor.setVolumeObject( m_model.import( Model::Method::Raycasting ) );
        raycasting_editor.apply( [&] ( kvs::TransferFunction tfunc )
        {
            m_model.tfuncs[Model::Method::Raycasting] = tfunc;
            ...
            auto& raycasting_cmap_bar = m_cmap_bars[Model::Method::Raycasting];
            raycasting_cmap_bar.setColorMap( tfunc.colorMap() );
            m_view.screen().redraw();
        } );
        raycasting_editor.show();

        auto& streamline_editor = m_editors[Model::Method::Streamline];
        streamline_editor.setTransferFunction( m_model.tfuncs[Model::Method::Streamline] );
        streamline_editor.setVolumeObject( m_model.import( Model::Method::Streamline ) );
        streamline_editor.apply( [&] ( kvs::TransferFunction tfunc )
        {
            m_model.tfuncs[Model::Method::Streamline] = tfunc;
            ...
            auto& raycasting_cmap_bar = m_cmap_bars[Model::Method::Streamline];
            raycasting_cmap_bar.setColorMap( tfunc.colorMap() );
            m_view.screen().redraw();
        } );
        streamline_editor.show();
...
private:
    ...
    void setVisible( const bool visible = true, const bool all = true )
    {
        ...
        if ( all )
        {
            ...
            m_cmap_bars[Model::Method::Raycasting].setVisible( visible );
            m_cmap_bars[Model::Method::Streamline].setVisible( visible );
        }
    }
    ...
    void setLayout( const int margin = 10 )
    {
        ...
        raycasting_cmap_bar.anchorToTop( &streamline_cmap_bar );
    }
};
